...
if(!['127.0.0.1','localhost','::1'].includes(host)&&!process.env.ADMIN_PASSWORD)throw Error('Defina ADMIN_PASSWORD para disponibilizar o painel na rede.');
const api=createBookingApi({dataDir:process.env.ORGANZZA_DATA_DIR||path.join(root,'data'),password:process.env.ADMIN_PASSWORD||''});
