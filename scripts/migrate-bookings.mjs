...
import {createFirestoreStorage,firebaseOptionsFromEnv} from '../booking-storage.mjs';
import {normalizeAppointment,validCustomTime,appointmentsOverlap} from '../booking-server.mjs';

let storage;
try {
  if(!process.env.FIREBASE_PROJECT_ID)throw Error('Configure as credenciais do Firebase no ambiente para importar os pedidos.');
  ...
  storage=createFirestoreStorage(firebaseOptionsFromEnv());
  ...
  const safe=error.code?'Não foi possível acessar o banco. Confira as credenciais do Firebase e a conexão.':error.message;
