...
let db,socket;const processes=[];
const databaseUrl='postgresql://postgres:postgres@127.0.0.1:5439/postgres?sslmode=disable';
function launch(port,extra={}) {
  const process=spawn(globalThis.process.execPath,['server.mjs'],{env:{...globalThis.process.env,NODE_ENV:'test',VERCEL:'1',HOST:'127.0.0.1',PORT:String(port),DATABASE_URL:databaseUrl,ADMIN_PASSWORD:password,SESSION_SECRET:secret,ORGANZZA_DATA_DIR:path.join(temporary,'unused-files'),...extra},stdio:'pipe',windowsHide:true});
...
try {
  db=await PGlite.create();socket=new PGLiteSocketServer({db,port:5439,host:'127.0.0.1',maxConnections:16});await socket.start();
  ...
  async function migrate() {const p=spawn(process.execPath,['scripts/migrate-bookings.mjs',originalFile],{env:{...process.env,DATABASE_URL:databaseUrl},stdio:'pipe',windowsHide:true});let output='';p.stdout.on('data',b=>output+=b);p.stderr.on('data',b=>output+=b);const code=await new Promise(r=>p.once('exit',r));assert.equal(code,0,output);return output;}
  ...
  const noDatabase=launch(4194,{DATABASE_URL:''});await started(4194,503);assert.equal((await request(4194,'/api/appointments','POST',payload)).status,503);await stop(noDatabase);
  ...
  console.log('OK: PostgreSQL via pg, duas instâncias, login após reinício, cookies seguros, expiração, concorrência sem duplicatas ou perda de pedidos, horários personalizados, aceite/recusa/remarcação, importação idempotente e Vercel sem gravação em arquivos.');
} finally {
  await Promise.all(processes.map(stop));await socket?.stop();await db?.close();
